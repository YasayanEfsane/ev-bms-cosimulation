...
summary = summarizeSimulationResults(results, false);
...
assert(isfinite(summary.distanceKm) && summary.distanceKm >= 0 && ...
    summary.energyDrawnKWh >= 0 && summary.regenerativeEnergyKWh >= 0, ...
    'Simulation KPI summary returned an invalid distance or energy value.');
assert(abs(summary.simulatedTimeS-cfg.sim.endTime) <= cfg.sim.logStep+eps, ...
    'Simulation KPI summary reported an inconsistent simulated duration.');
